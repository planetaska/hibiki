...
A strict-locals header turns a forgotten local into an error at render time instead of a `nil` three lines later — worth it for any partial that two code paths render, which is every partial a render effect touches (the controller renders it first, the effect re-renders it after).
